...
@description('CosmosDB Account name for data plane RBAC (optional - leave empty to skip)')
...
// =============================================================================
// CosmosDB requires a separate data plane RBAC assignment for reading/writing data.
// The "DocumentDB Account Contributor" role only grants control plane access.
// This module assigns "Cosmos DB Built-in Data Contributor" for actual data operations.
// =============================================================================

module cosmosDbDataPlaneRole 'modules/cosmosdb-data-plane-role.bicep' = if (!empty(cosmosDbAccountName)) {
  name: 'cosmosdb-data-plane-role'
  params: {
...
// CosmosDB Data Plane outputs (only if configured)
output COSMOSDB_DATA_PLANE_ROLE_ASSIGNED bool = !empty(cosmosDbAccountName)
